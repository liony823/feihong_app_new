...
import 'package:feihong/utils/http_util.dart';
import 'package:feihong/utils/logger.dart';
...
class Apis {
  ...
  static Future<bool> login(String uid, String token) async {
    try {
      await HttpUtil.post("$apiURL/user/login", data: {
        'uid': uid,
        'token': token,
        'device_flag': 0,
        'device_level': 1
  ...
  static syncMsgExtra(String channelId, int channelType, int version) async {
    ...
    try {
      final response = await HttpUtil.post('$apiURL/message/extra/sync', data: {
        'login_uid': uid,
  ...
  static Future<bool> syncConversation(String lastSsgSeqs, int msgCount,
    ...
    try {
      final data = await HttpUtil.post('$apiURL/conversation/sync', data: {
  ...
  static Future<bool> syncChannelMsg(
    ...
    try {
      final data = await HttpUtil.post('$apiURL/message/sync', data: {
        "login_uid": DataSp.uid, // 当前登录用户uid
